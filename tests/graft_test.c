...
static void copy_test_1(void *fdt)
{
    ...
    {
        FAIL("Expected no-space error when copying before resizing");
    }
    fdt_open_into(dst_fdt, dst_fdt, 4096);
    rc = fdt_copy_tree(fdt, dst_fdt, "/a");
    if (rc < 0)
    {
...
{
    void *src_fdt, *dst_fdt, *expanded_dst;
    int rc, src_size, dst_size;

    src_fdt = load_blob(a);
    dst_fdt = load_blob(b);
    src_size = fdt_totalsize(src_fdt);
    dst_size = fdt_totalsize(dst_fdt);
    expanded_dst = malloc(dst_size+src_size);
    fdt_open_into(dst_fdt, expanded_dst, dst_size+src_size);
    free(dst_fdt);
    rc = fdt_copy_tree(src_fdt, expanded_dst, "/firmware");
    if (rc < 0)
    {
        FAIL("Error copying /firmware: %s", fdt_strerror(rc));
    }
    rc = fdt_copy_tree(src_fdt, expanded_dst, "/reserved-memory");
    if (rc < 0)
    {
        FAIL("Error copying /reserved-memory: %s", fdt_strerror(rc));
    }
    save_blob(dst, expanded_dst);
    free(expanded_dst);
    free(src_fdt);
}
